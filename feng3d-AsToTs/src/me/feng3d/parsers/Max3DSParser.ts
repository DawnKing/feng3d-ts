...
//					}
		...
		private applySmoothGroups(vertices:VertexVO[], faces:FaceVO[])
		{
			...
			var vGroups:Vector.<number[]> = new Vector.<number[]>(numVerts, true);
			...
			// clone vertices
			var vClones:Vector.<number[]> = new Vector.<number[]>(numVerts, true);
			for (i = 0; i < numVerts; i++)
			{
				if ((len = vGroups[i].length) < 1)
					continue;
				var clones:number[] = new number[](len, true);
				vClones[i] = clones;
				clones[0] = i;
