module feng3d
{
	...
	export class Max3DSParser extends ParserBase
	{
		 ...
		 * @inheritDoc
		 */
		protected override function startParsing(frameLimit:number)
		{
		...
		protected override function proceedParsing():boolean
		{
			...
			while (hasTime())
			{
				...
				else if (this._cur_obj && this._byteData.position >= this._cur_obj_end)
				{
					...
					this._cur_obj_end = uint.MAX_VALUE;
					this._cur_obj = null;
				}
...
//					}
		...
		private parseVertexList()
		{
			...
			while (i < len)
			{
				var x:number, this.y:number, this.z:number;
		...
		private parseFaceList()
		{
			...
			count = this._byteData.readUnsignedShort();
			this._cur_obj.indices = new uint[](count * 3, true);
			...
			this._cur_obj.smoothingGroups = new uint[](count, true);
		}
		...
		private constructObject(obj:ObjectVO, pivot:Vector3D = null):Container3D
		{
			if (obj.type == AssetType.MESH)
			{
				...
				obj.indices = new uint[](faces.length * 3, true);
		...
		private applySmoothGroups(vertices:VertexVO[], faces:FaceVO[])
		{
			...
			var vGroups:Vector.<uint[]> = new Vector.<uint[]>(numVerts, true);
			...
			for (i = 0; i < numFaces; i++)
			{
				...
				for (j = 0; j < 3; j++)
				{
					var groups:uint[] = vGroups[(j == 0) ? face.a : ((j == 1) ? face.b : face.c)];
			...
			// clone vertices
			var vClones:Vector.<uint[]> = new Vector.<uint[]>(numVerts, true);
			for (i = 0; i < numVerts; i++)
			{
				if ((len = vGroups[i].length) < 1)
					continue;
				var clones:uint[] = new uint[](len, true);
				vClones[i] = clones;
				clones[0] = i;
...
//				MultiPassMaterialBase(mat).specularColor = this._cur_mat.specularColor;
		...
		private readColor():number
		{
			var cid:number;
			var len:number;
			var r:number, g:number, this.b:number;
